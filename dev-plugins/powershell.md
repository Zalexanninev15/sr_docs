...
5\. Основной код необходимо написать в файле **main.ps1** (файл кода написанный для _PowerShell_). Можно создать и редактировать с помощью _Windows PowerShell ISE_ (хотя он преднапредназначен для устаревшей версии PowerShell, но инструмент всё ещё отличный, когда дело касается PowerShell любой версии), который встроен в Windows (`C:\Windows\system32\WindowsPowerShell\v1.0\powershell_ise.exe`), или воспользоваться другим удобным для вас редактором кода, рекомендую Visual Studio Code.
